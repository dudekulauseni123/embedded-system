...
\fi
\begin{abstract}
Through this manual, we learn how to measure an unknown resistance through ESP32 and monitoring Unknown resistance through ESP32 webserver.
\end{abstract}
...
\begin{table}[!h]
\centering
\input{./vaman/vaman-esp/webserver/figs/components.tex}
\caption{Components}
\label{table:webserver/components}
\end{table}
...
\begin{lstlisting}
vaman/vaman-esp/webserver/codes
\end{lstlisting}
\item After flashing the code to ESP32, the board will be connected to the wifi credentials provided.
\item Now connect the same WiFi credentials to the laptop for accessing the IP address, which can be accessed by 
\begin{lstlisting}
ifconfig
nmap -sn 192.168.x.x/24
\end{lstlisting}
\item Change the IP address in the second command accordingly with the IP address provided by first command.
\item By the above commands the IP address of ESP32 will be diplayed.
\item Now the ESP32 will be hosting a webserver
\item Inorder to access the webserver type the IP address of the ESP32 in the web browser.
\item In the website loaded by the IP address of ESP32 the Unknown resitance is displayed as shown in Fig. \ref{fig:website}

\begin{figure}[!ht]
\centering
\includegraphics[width=\columnwidth]{./vaman/vaman-esp/webserver/figs/website.jpg}
\caption{Website}
\label{fig:website}
\end{figure}
